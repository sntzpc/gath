...
function route_(action, p, token){
  switch(action){
    ...
    case 'public.liveLocations': return public_liveLocations_(p);
...
// Ambil meta peserta (region/unit) untuk kebutuhan Live Map.
// Prioritas: attendance (yang sudah hadir) -> participants.
...
function liveMetaMap_(){
  ...
  try{
    const ps = getAll_(SH.participants);
    ps.forEach(r=>{
      const nik = String(r.nik||'').trim();
      if(!nik) return;
      if(meta[nik]) return;
...
function admin_liveLocations_(p, u){
  ...
  const rows = liveRows.map(r=>{
    ...
    return {
      nik: nik,
      name: String(r.name||'') || String(m.name||''),
...
// ---- Public live map (tanpa token) ----
function public_liveLocations_(p){
  // Untuk kebutuhan Live Map standalone tanpa autentifikasi.
  // Data yang dikembalikan sama seperti admin.liveLocations.
  return admin_liveLocations_(p, null);
}
